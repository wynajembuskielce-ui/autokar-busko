    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center px-4">
      <div className="bg-gray-800 p-8 rounded-2xl w-full max-w-sm">
        ...
        <input type="password" placeholder="Hasło..." value={haslo} onChange={e => setHaslo(e.target.value)}
          ...
          className="w-full p-3 rounded-xl bg-gray-700 text-white mb-3 outline-none border-2 border-gray-600 focus:border-[#f59e0b]" />
        <button onClick={() => haslo === HASLO ? setZalogowany(true) : alert('Złe hasło!')}
          className="w-full bg-[#f59e0b] text-black font-bold py-3 rounded-xl hover:opacity-90">
          Wejdź
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-gray-900 text-white py-8 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="bg-gray-800 rounded-2xl p-6 mb-4 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-[#f59e0b]">🦁 Panel Wycen — Rafał</h1>
        ...
        <div className="bg-gray-800 rounded-2xl p-5 mb-3">
          <h2 className="font-bold mb-3 text-[#f59e0b]">⛽ Aktualna cena ON (zł/litr)</h2>
          <input type="number" step="0.01" value={customFuel} onChange={e => setCustomFuel(Number(e.target.value))}
            className="w-full p-3 bg-gray-700 rounded-xl outline-none border-2 border-gray-600 focus:border-[#f59e0b] text-xl font-bold" />
          <p className="text-xs text-gray-500 mt-1">Sprawdź aktualną cenę ON na orlen.pl</p>
        </div>

        <div className="bg-gray-800 rounded-2xl p-5 mb-3">
          <h2 className="font-bold mb-3 text-[#f59e0b]">🚌 Pojazd i klasa</h2>
          <select value={vehicleId} onChange={e => setVehicleId(e.target.value)}
            className="w-full p-3 bg-gray-700 rounded-xl mb-3 outline-none border-2 border-gray-600 focus:border-[#f59e0b]">
            {pricingConfig.vehicles.map(v => <option key={v.id} value={v.id}>{v.name_pl}</option>)}
          </select>
          <div className="grid grid-cols-3 gap-2">
            {['standard','biznes','vip'].map(t => (
              <button key={t} onClick={() => setTier(t)}
                className={`p-2 rounded-xl border-2 font-bold text-sm capitalize transition ${tier === t ? 'border-[#f59e0b] bg-amber-900/30 text-[#f59e0b]' : 'border-gray-600 hover:border-gray-500'}`}>
        ...
        <div className="bg-gray-800 rounded-2xl p-5 mb-3">
          <h2 className="font-bold mb-3 text-[#f59e0b]">📍 Trasa i dni</h2>
          ...
          {routeId === 'custom' && (
            <input type="number" value={customKm} onChange={e => setCustomKm(Number(e.target.value))}
              placeholder="Km w jedną stronę" className="w-full p-3 bg-gray-700 rounded-xl mb-3 outline-none border-2 border-gray-600 focus:border-[#f59e0b]" />
          )}
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => setDays(days+1)} className="w-8 h-8 rounded-full bg-[#f59e0b] font-bold hover:opacity-90 text-black">+</button>
            <span className="text-gray-400 text-sm ml-4">Łącznie: {kmTotal} km</span>
          </div>
        </div>

        <div className="bg-gray-800 rounded-2xl p-5 mb-3">
          <h2 className="font-bold mb-3 text-[#f59e0b]">🌙 Dopłaty</h2>
          <div className="space-y-2">
            {[
              ...
              <label key={i} className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={item.val} onChange={e => item.set(e.target.checked)} className="w-4 h-4 accent-amber-500" />
        ...
        <div className="bg-gray-800 rounded-2xl p-5 mb-3">
          <h2 className="font-bold mb-3 text-[#f59e0b]">💡 Dodatkowy zysk / rabat (zł)</h2>
          <input type="number" value={profit} onChange={e => setProfit(Number(e.target.value))}
            className="w-full p-3 bg-gray-700 rounded-xl outline-none border-2 border-gray-600 focus:border-[#f59e0b]"
            placeholder="0 = standardowa cena, minus = rabat" />
        </div>

        <div className="bg-[#1a1a1a] border-2 border-[#f59e0b] rounded-2xl p-6">
          <h2 className="font-bold text-lg mb-4 text-[#f59e0b]">📊 Kalkulacja</h2>
          <div className="space-y-1 text-sm mb-4">
            ...
            <div className="border-t border-gray-700 pt-3 mt-2">
              ...
              <div className="flex justify-between text-2xl mt-1"><span className="text-[#f59e0b]">Brutto:</span><span className="font-bold text-[#f59e0b]">{totalBrutto.toFixed(2)} zł</span></div>
